...
const USAGE_API_URL = 'https://uas-reporting.cfapps.eu10.hana.ondemand.com/reports/v1/monthlyUsage';
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const usageUrl = `${USAGE_API_URL}?fromDate=${fromDate}&toDate=${toDate}`;
    ...
    if (!usageResponse.ok) {
      ...
      throw new Error(`Failed to fetch usage data: ${usageResponse.statusText} - ${errorText}`);
    }
    ...
    console.error('Error in sap-usage-proxy:', error);
